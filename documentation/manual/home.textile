...
One big plus of Scala is the ability to define some default values for this parameter:
...
p(note). Usually, it's recommended to use the companion object form to provide query functionality (ie. User.count). The main advantage of the more generic *QueryOn* approach is that it's working with both *scala and java* models. 
